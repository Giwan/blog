...
export const description = `Let's build a pagination component. It should adhere to some basic UI interaction requirements. The goal is deliver an extra light component.`;
...
It shows always shows five page buttons. Left and right of those are buttons to move by one page. 
In addition there are buttons for the first and last pages. 
...
```
With a total number of 100 pages, the component should only show 5 at any given time. The selected page is centered if possible.
At the end or beginning of the list, there needs to be either 4 pages to the left or right of the selected page. 

<PageContainer totalPages={100} startPage={1} />
...
There fore it's known which values will be converted to HTML. There are no unsafe values being passed in this case. 
...
Depending on the current page value, several buttons are created or not created. 
If the user is at the beginning, on page one, then no buttons are created before it.
...
The current page is 100. The last page. Four buttons are created before it. 96, 97, 98 and 99. No buttons are created after it. 
...
While the design here needs work, I believe the functionality matches the requirements. 
