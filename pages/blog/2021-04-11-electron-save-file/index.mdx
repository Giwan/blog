...
export const description = `After setting up a basic ElectronJS, this article discusses the preload.js script.
It's used to communicate between the main electron process and the unsafe renderer process. The latter is not allowed to access the fs API for security reasons.`;
...
Tt's possible to disable. Don't do it though. 
...
[The security concerns and mitigation strategies are well documented on electronjs.org](https://www.electronjs.org/docs/tutorial/security)
...
Now that the message text is available in the main process the [dialog api](https://www.electronjs.org/docs/api/dialog) can be used to save the text to disk.
